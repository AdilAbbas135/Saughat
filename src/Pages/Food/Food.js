...
const FoodsPage = () => {
  ...
  const FetchFood = async () => {
    await axios
      .get(`${process.env.REACT_APP_BACKEND_URL}/find/food`)
      .then((response) => {
  ...
  return (
    <div className="relative border-2 border-gray-200 rounded-md overflow-hidden">
      ...
      <div className="p-5">
        ...
        <ul className="mt-1 flex items-center gap-2 text-sm">
          {tution?.Items.map((item, index) => {
            return (
              <li
                key={index}
                className="text-main_bg_color bg-main_color_2 px-3 py-1 font-bold"
              >
                {item}
              </li>
            );
      ...
      <div className="border-t-2 border-gray-200">
        <div className="flex items-center justify-around">
          ...
          <p className=" text-text_color_secondary_2 py-2">
            Starting at:{" "}
            <span className="text-[16px] font-bold text-text_color_secondary">
              PKR {tution?.Price}
            </span>
          </p>
        </div>
