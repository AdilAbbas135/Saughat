...
const SingleTution = () => {
  ...
  const TutionId = location.pathname.split("/").slice(-1);
  const [loading, setloading] = useState(true);
  const [Tution, setTution] = useState({});

  const FetchTution = async () => {
    await axios
      .get(
        `${process.env.REACT_APP_BACKEND_URL}/find/tutions/singleTution?TutionId=${TutionId}`
      )
      .then((response) => {
            ...
            <div className="mt-10 w-full max-w-7xl mx-auto">
              <div className="grid grid-cols-10 gap-10">
                <div className="col-span-7">
                  <h1 className="font-[700] text-3xl text-text_color uppercase">
                    {Tution.Title}
                  </h1>
