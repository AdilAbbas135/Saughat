...
import { FetchProfile } from "../../Redux/StudentDashboard";
...
const DashboardUI = (props) => {
  ...
  const dashboard = useSelector((state) => state.StudentDashboard);
  const error = useSelector((state) => state.StudentDashboard.error);
  const [showSidebar, setshowSidebar] = useState(true);

  useEffect(() => {
    if (!dashboard.isProfileFetched && !dashboard?.Student?._id) {
