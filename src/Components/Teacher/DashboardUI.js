...
import { FetchProfile } from "../../Redux/TeacherDashboard";
...
const DashboardUI = (props) => {
  ...
  const dashboard = useSelector((state) => state.TeacherDashboard);
  const error = useSelector((state) => state.TeacherDashboard.error);
  const [showSidebar, setshowSidebar] = useState(true);

  useEffect(() => {
    if (!dashboard.isProfileFetched && !dashboard?.Teacher?._id) {
