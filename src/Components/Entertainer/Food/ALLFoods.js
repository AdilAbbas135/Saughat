...
const AllFood = () => {
  ...
  const FetchFood = () => {
    ...
    axios
      .post(
        `${process.env.REACT_APP_BACKEND_URL}/hall-manager/food`,
        {},
        { headers: { token: token } }
      )
      .then((result) => {
        console.log(result);
        setallTutions(result.data?.Food);
        setloading(false);
      })
      .catch((err) => {
        console.log(err);
  ...
  return (
    <div className="">
      <div className="mb-2 flex justify-between">
        <h1 className="font-bold text-text_color text-2xl mb-5 pl-1 border-l-[5px] border-hover_color uppercase">
          Your MENU
        </h1>
        <Link to="/hall-manager/food/add-food">
          <Button className="px-5 py-2 text-[16px] h-fit bg-hover_color shadow-none hover:shadow-none  font-semibold rounded-md flex items-center justify-center hover:scale-105 transition-all">
            <IoMdAddCircleOutline className="mr-1" /> Add Menu
          </Button>
        </Link>
      </div>
                      ...
                      {elem.Items.length > 0 ? (
                        elem.Items.map((item, index) => {
                          return (
                            <div
                              key={index}
                              className="flex flex-wrap gap-2 py-2 w-fit px-3 bg-[#d1ecf1] rounded-md text-sm"
                            >
                              {item}
                            </div>
                          );
                        ...
                        <>
                          <p className="text-red-500 text-sm italic">
                            No Items have added into this menu{" "}
                          </p>
                        </>
                      )}
                    </div>
